module CSSP {
    export class TestInput {
        ...
        public DoTest: Function = ($cssPath: JQuery, CurrentCSSPath: string): void => {
            ...
            test(CurrentCSSPath + " [" + $cssPath.attr("name") + "]", () => {
            ...
            test(CurrentCSSPath + " [" + $cssPath.attr("name") + "] Validate ", () => {
                ...
                for (var propName in $cssPath.rules()) {
                    ...
                    for (var propName2 in $cssPath.rules()[propName]) {
                        switch (propName2) {
                            case "url":
                                {
                                    equal(eval("$cssPath.rules()." + propName + "." + propName2),
                                        $cssPath.rules()[propName][propName2].replace(/\+/g, "\/").replace(Globalize.culture.name, Globalize.culture.name),
                                        "[" + propName + "." + propName2 + " exist for input value of " + $cssPath.rules()[propName][propName2].replace(/\+/g, "\\/").replace(Globalize.culture.name, Globalize.culture.name) + "]");
                                }
                                break;
                            case "data":
                                {
                                    equal(eval("typeof($cssPath.rules()." + propName + "." + propName2 + ")"),
                                        "object",
                                        "[" + propName + "." + propName2 + " exist for input]");
                                }
                                break;
                            default:
                                {
                                    if (propName == "range" || propName == "rangelength") {
                                        ...
                                        equal(eval("$cssPath.rules()." + propName + "." + propName2),
                                            $cssPath.rules()[propName][propName2].replace(/\+/g, "\\/"),
                                            "[" + propName + "." + propName2 + " exist for input value of " + $cssPath.rules()[propName][propName2].replace(/\+/g, "\\/") + "]");
                                    }
                                }
                                break;
                        }
                            ...
                            for (var propName3 in $cssPath.rules()[propName][propName2]) {
                                if (propName2 == "url") {
                                    ...
                                    switch (propName3) {
                                        default:
                                            {
                                                equal(eval("typeof($cssPath.rules()." + propName + "." + propName2 + "." + propName3 + ")"),
                                                    typeof ($cssPath.rules()[propName][propName2][propName3]),
                                                    "[" + propName + "." + propName2 + "." + propName3 + " exist for input type of " + typeof ($cssPath.rules()[propName][propName2][propName3]) + "]");
                                            }
                                            break;
                                    }
                                }
